...
Text-to-Speech (TTS): Integrated vocal listening assistance for both source inputs and translated outputs using the native browser Web Speech API.

Smart UI Utilities:

Quick-swap language utility (⇄) to instantly switch between selected source and target languages.

One-click dynamic clipboard copy function (📋 Copy) with a visual confirmation alert.
...
Since this project relies completely on native client-side web technologies and a free translation framework, no complex setups or API keys are required!

Method A (Recommended): Open the project folder in Visual Studio Code, right-click index.html, and select "Open with Live Server".
...
├── script.js           # Core script running the network fetch requests and TTS utilities
├── config.js           # Local configuration setup file
